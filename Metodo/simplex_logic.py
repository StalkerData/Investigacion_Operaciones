...
class SimplexSolver:
    ...
    def solve(self):
        ...
        while True:
            ...
            if col_piv is None:
                ...
                estado_actual["solucion"] = self._interpretar_solucion(
                    tabla, current_basis
                )
            ...
            estado_actual["mensaje"] = (
                f"Entra {var_entra} (Col {col_piv+1}) y Sale {var_sale} (Fila {fila_piv})"
            )
            self.history.append(estado_actual)
    ...
    def _crear_tabla_inicial(self):
        tabla = np.zeros(
            (self.num_restricciones + 1, self.num_vars + self.num_restricciones + 1)
        )
        tabla[1:, : self.num_vars] = self.A
        tabla[1:, self.num_vars : self.num_vars + self.num_restricciones] = np.eye(
